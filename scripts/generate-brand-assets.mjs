...
const receiptPath = path.join(publicDir, 'cafe-receipt-moment.png')
...
const receiptSize = 500
const receipt = await sharp(receiptPath)
  .resize(receiptSize, receiptSize, { fit: 'cover' })
  .composite([{ input: Buffer.from(`<svg width="${receiptSize}" height="${receiptSize}"><rect width="${receiptSize}" height="${receiptSize}" rx="24" fill="white"/></svg>`), blend: 'dest-in' }])
  .png()
  .toBuffer()
...
const socialLayer = Buffer.from(`
  <svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="76" y="291" fill="#2d2925" font-family="Arial, Apple SD Gothic Neo, sans-serif" font-size="40" font-weight="700">영수증 찍으면,</text>
