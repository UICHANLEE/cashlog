...
앱에서는 이메일/비밀번호 회원가입·로그인, Google·카카오 간편 로그인, 비밀번호 없는 메일 링크 로그인을 모두 지원합니다. 회원가입 시 만 14세 이상, 이용약관, 개인정보, 사진·시간 처리 필수 동의와 위치정보 선택 동의를 구분해 받고 동의 버전을 저장합니다. 로그인하면 로컬 기록과 원격 기록을 병합하고, 선택한 캐릭터 프로필과 사진 보관본도 계정에 저장합니다.
...
개인정보 문의 주소로 `privacy@cashlog.ai.kr`를 공개하고 있으므로 배포 전에 Cloudflare Email Routing 또는 사용하는 메일 서비스에서 실제 수신 가능한 전달 주소로 연결합니다.
