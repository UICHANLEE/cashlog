...
조회 화면에는 최근 24시간 이벤트, 활성 세션, 평균 화면 체류시간, 첫 행동까지 걸린 시간, 화면별 첫 행동률과 버튼별 클릭 순위가 표시됩니다. 기간과 이벤트 종류로 원본 이벤트를 필터링할 수도 있습니다.
...
3. Vercel 프로젝트의 Settings → Environment Variables에 아래 서버 전용 변수를 Production, Preview에 추가합니다.
...
4. Vercel을 다시 배포합니다. 빌드 시점 이후에 추가한 환경변수는 기존 배포에 자동 적용되지 않습니다.
5. `CASHLOG_ADMIN_EMAILS`에 적은 이메일로 `/login.html`에서 로그인합니다.
6. `/uichan`으로 이동해 연결 상태와 사용자 활동 로그를 확인합니다.
